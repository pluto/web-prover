...
use ring::{constant_time, digest::Context};
...
pub(super) async fn handle_server_hello(
    ...
    // TODO: REMOVE ME!!! 
    {
        ...
        // witness values
        let ms = BASE64_STANDARD
            .decode("vC4GIYWwM1k4WJnIuW29+OpFLGJdzIJeuWnkE93wYQM=")
            .unwrap();
        let arr: [u8; 32] = ms[..32].try_into().unwrap();
        let _ = cx.common.backend.set_pre_master_secret(arr);

        // HANDSHAKE layer
        let h2 = BASE64_STANDARD
            .decode("sGq3ZD04fmlO/DSPh/6LvBWSMTjpAeETrGEyJ0NvHXA=")
            .unwrap();
        cx.common.backend.set_hs_hash_client_key_exchange(h2)?;
        cx.common
            .backend
            .set_encrypt_decrypt(EncryptMode::Handshake)?;

        // APPLICATION layer
        let h3 = BASE64_STANDARD
            .decode("T2oRxmufMZ+212X4G+qyUt7tB73Xmun+BsTfPz9u9bE=")
            .unwrap();
        cx.common.backend.set_hs_hash_client_key_exchange(h3)?;
        cx.common
            .backend
            .set_encrypt_decrypt(EncryptMode::Application)?;
    }
    ...
    // overwrites ems_seed with transcript hash
    cx.common
        .backend
    ...
    // Inform the record layer that our keys are ready.
    ...
    // Note: RusTls outputs the KeySchedule as part of this process. That
    // schedule is then passed into future steps and updated along the way
    // to carry forward keys. In the TLSN fork, KeySchedule has been removed.
...
impl State<ClientConnectionData> for ExpectCertificateVerify {
    async fn handle(
        ...
        // cx.common.backend.set_witness_h7(h7.as_ref())?;
        cx.common.backend.set_witness_h7(
            &BASE64_STANDARD
                .decode("WSZc7DcCkCHYCGpbdprCwa1ZZFxdPSjKyU33hYWkSk4=")
                .unwrap(),
        )?;
...
impl State<ClientConnectionData> for ExpectFinished {
    async fn handle(
        ...
        cx.common
            .backend
            .set_hs_hash_client_key_exchange(handshake_hash.as_ref().to_vec())?;

        cx.common
            .backend
            .set_encrypt_decrypt(EncryptMode::Application)?;

        // These reset the sequence number. This is crucial when switching enc/dec modes.
        cx.common.record_layer.set_message_encrypter();
        cx.common.record_layer.set_message_decrypter();

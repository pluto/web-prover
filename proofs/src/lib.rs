...
use client_side_prover::{
  fast_serde::{self, FastSerde}, provider::GrumpkinEngine, spartan::batched::BatchedRelaxedR1CSSNARK, supernova::{snark::CompressedSNARK, PublicParams, TrivialCircuit}, traits::{Dual, Engine, Group}
};
use ff::Field;
...
use std::io::Cursor;
use client_side_prover::fast_serde::{SerdeByteTypes, SerdeByteError};
...
impl FastSerde for ProvingParams {
  ...
  fn from_bytes(bytes: &Vec<u8>) -> Result<Self, SerdeByteError> {
    ...
    let aux_params = Self::read_section_bytes(&mut cursor, 1).map(|bytes| AuxParams::from_bytes(&bytes))?.unwrap();
    let vk_digest_primary = Self::read_section_bytes(&mut cursor, 2).map(|bytes| <E1 as Engine>::Scalar::from_bytes(&bytes.try_into().unwrap()))?.unwrap();
    let vk_digest_secondary = Self::read_section_bytes(&mut cursor, 3).map(|bytes| <Dual<E1> as Engine>::Scalar::from_bytes(&bytes.try_into().unwrap()))?.unwrap();
...
impl ProvingParams {
  /// Method used externally to initialize all the backend data needed to create a verifiable proof with
  ...
  pub fn new(aux_params: AuxParams, prover_key: ProverKey) -> Result<ProvingParams, ProofError> {   
    ...
    // debug!("initialized pk_primary.digest={:?}, pk_secondary.digest={:?}", hex::encode(pk.pk_primary.vk_digest.to_bytes()), hex::encode(pk.pk_secondary.vk_digest.to_bytes()));
    Ok(ProvingParams { 
      aux_params,

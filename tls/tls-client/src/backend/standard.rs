...
impl RustCryptoBackend {
    ...
    pub fn key_expansion_tls12(
        ...
        self.logger.set_secret(String::from("ms"), ms.to_vec());
...
impl Backend for RustCryptoBackend {
    ...
    fn set_encrypt_decrypt(&mut self, _mode: EncryptMode) -> Result<(), BackendError> {
        // NOTE: These are not implemented because the set_server_share
        // method below derives and records the keys, with set_encryptor and set_decryptor.
        todo!()
    }
    ...
    fn set_hs_hash_server_hello(&mut self, _hash: Vec<u8>) -> Result<(), BackendError> {
        Ok(())
    }

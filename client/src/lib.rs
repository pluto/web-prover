...
pub async fn prover_inner(config: Config) -> Result<TlsProof, errors::ClientErrors> {
  ...
  // // TODO: These three lines with target_url should probably throw a well-defined error instead
  // of // causing panic.
  ...
  // let session_id = "c655ee6e-fad7-44c3-8884-5330287982a8"; // TODO random hardcoded UUID4. notary
  // does not need it anymore. prover_config.id(session_id).server_dns(target_host).
  // root_cert_store(root_store); prover_config.max_transcript_size(
  ...
  // //---------------------------------------------------------------------------------------------------------------------------------------/
  // / // TODO: This will likely not compile without websocket feature  on wasm since this is a
  // tokio // tcpstream. ALSO, this does noit actually provide a websocket feature for non-wasm.
  // //
  ...
  //   hyper::client::conn::handshake(mpc_tls_connection.compat()).await?;
  ...
  // // TODO: This could be a source of error as the mapping now just holds a single string, so I
  // will // leave commented out code here.
  ...
  //     debug!("Request OK");
  //   },
  //   Err(e) if e.is_incomplete_message() => println!("Response: IncompleteMessage (ignored)"), /*
  // TODO */   Err(e) => panic!("{:?}", e),
  // };
  ...
  //   let mut client_socket = connection_receiver.await.unwrap()?.io.into_inner(); // TODO fix
  // unwrap   client_socket.close().await?;
  // }
  ...
  // //---------------------------------------------------------------------------------------------------------------------------------------/
  // /
  panic!("client::prover_inner todo")
}

...
pub async fn verify(proof: &str, notary_pubkey_str: &str) -> Result<String, JsValue> {
  ...
  #[cfg(feature = "tracing")]
  info!("!@# notary_pubkey {}, {}", notary_pubkey_str, notary_pubkey_str.len());
  session
  ...
  #[cfg(feature = "tracing")]
  {
    ...
    info!(
      ...
      session_info.server_name, _time
    );
  ...
  let result = VerifyResult {
    ...
    sent:        String::from_utf8(sent.data().to_vec())
      .map_err(|e| JsValue::from_str(&format!("Could not convert sent data to string: {:?}", e)))?,
    recv:        String::from_utf8(recv.data().to_vec())
      .map_err(|e| JsValue::from_str(&format!("Could not convert recv data to string: {:?}", e)))?,
  };

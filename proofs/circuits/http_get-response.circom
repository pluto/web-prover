...
template LockHTTPResponse(DATA_BYTES, maxContentLength, versionLen, statusLen, messageLen, headerNameLen1, headerValueLen1) {
    // Raw HTTP bytestream
    signal input step_in[DATA_BYTES]; // Changed this from `data`
    ...
    signal output step_out[maxContentLength]; // Changed this from `body`

    signal bodyMask[DATA_BYTES];

    // Check first version byte
    signal versionIsEqual[versionLen];
    versionIsEqual[0] <== IsEqual()([step_in[0],version[0]]);
    versionIsEqual[0] === 1;
    ...
    bodyMask[0] <== step_in[0] * State[0].next_parsing_body;
    ...
    headerNameValueMatch1[0]    <== HeaderFieldNameValueMatch(DATA_BYTES, headerNameLen1, headerValueLen1)(step_in, header1, value1, 0);
    hasMatchedHeaderValue1      += headerNameValueMatch1[0];

    for(var data_idx = 1; data_idx < DATA_BYTES; data_idx++) {
        State[data_idx]                       = HttpStateUpdate();
        State[data_idx].byte                <== step_in[data_idx];
        ...
        // Mask if parser is in the body of response
        bodyMask[data_idx] <== step_in[data_idx] * State[data_idx].next_parsing_body;

        // Check remaining version bytes
        if(data_idx < versionLen) {
            versionIsEqual[data_idx] <== IsEqual()([step_in[data_idx], version[data_idx]]);
        ...
        headerNameValueMatch1[data_idx] <== HeaderFieldNameValueMatch(DATA_BYTES, headerNameLen1, headerValueLen1)(step_in, header1, value1, data_idx);
    ...
    step_out <== SelectSubArray(DATA_BYTES, maxContentLength)(bodyMask, bodyStartingIndex[DATA_BYTES-1]+1, DATA_BYTES - bodyStartingIndex[DATA_BYTES-1]);
    ...
    signal statusMatch <== SubstringMatchWithIndex(DATA_BYTES, statusLen)(step_in, status, status_start_counter + 1);
    statusMatch        === 1;
    statusLen          === status_end_counter - status_start_counter - 1;

    // Check message is correct by substring match and length check
    signal messageMatch <== SubstringMatchWithIndex(DATA_BYTES, messageLen)(step_in, message, status_end_counter + 1);
    messageMatch        === 1;
    // -2 here for the CRLF
...
component main { public [step_in] } = LockHTTPResponse(89,18,8,3,2,12,16);
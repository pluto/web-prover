...
Usage of ./vanilla-go-app:
  ...
  -listen string
        Listen for connections on host:port (default ":8065")
...
./vanilla-go-app -listen :8065
./vanilla-go-app -listen :8065 -tls-cert-path ./cert -tls-key-path ./key

# Returns 200
curl http://localhost:8065

# Returns 200 and fixed size binary body
curl http://localhost:8065/bin/0KB
curl http://localhost:8065/bin/1KB
curl http://localhost:8065/bin/10KB
curl http://localhost:8065/bin/100KB
curl http://localhost:8065/bin/1000KB

# Returns 200 and fixed size text body
curl http://localhost:8065/plain/0KB
curl http://localhost:8065/plain/1KB
curl http://localhost:8065/plain/10KB
curl http://localhost:8065/plain/100KB
curl http://localhost:8065/plain/1000KB

# Read and discard the body, returns 204
curl http://localhost:8065/readall -X POST -d 'foobar'

# Reads the body and returns it with status 201
curl http://localhost:8065/echo -X POST -d 'foobar'

# Sleeps for some specified time and returns 202
curl http://localhost:8065/sleep?ms=500 

# Sleeps for some specified time and then closes the TCP connection
curl http://localhost:8065/timeout?ms=500

# Returns 200 and returns request with headers and body as seen by server
curl http://localhost:8065/debug-request

# Returns 200 and dumps request to stdout
curl http://localhost:8065/stdout
```

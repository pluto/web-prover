...
pub async fn setup_tcp_connection(
  ...
  let connection_task = tokio::spawn(connection.without_shutdown()).await?;
  ...
  let hyper::client::conn::http1::Parts { io: notary_tls_socket, .. } = connection_task?;
  ...
  let client_socket =
    tokio::net::TcpStream::connect((config.target_host()?, config.target_port()?)).await?;

  let (mpc_tls_connection, prover_fut) = prover.connect(client_socket.compat()).await?;

  let prover_task = tokio::spawn(prover_fut).await?;
  ...
  let client_socket = connection_task?.io.into_inner();

  use futures::AsyncWriteExt;
  client_socket.compat().close().await?;

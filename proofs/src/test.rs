...
use arecibo::{
  ...
  provider::{
    poseidon::PoseidonConstantsCircuit, Bn256EngineIPA, PallasEngine, Secp256k1Engine, VestaEngine,
  },
  supernova::{
    circuit::{StepCircuit, TrivialSecondaryCircuit, TrivialTestCircuit},
    NonUniformCircuit,
  },
...
where F: PrimeField
{
  ...
  fn synthesize<CS: ConstraintSystem<F>>(
    ...
    // Consider a cubic equation: `x^3 + x + 5 = y`, where `x` and `y` are respectively the input
    // and output.
    let x = &z[0];
...
where F: PrimeField
{
  ...
  fn synthesize<CS: ConstraintSystem<F>>(
    ...
    // Consider an equation: `x^2 + x + 5 = y`, where `x` and `y` are respectively the input and
    // output.
    let x = &z[0];
    let x_sq = x.square(cs.namespace(|| "x_sq"))?;
    let y = AllocatedNum::alloc(cs.namespace(|| "y"), || {
...
// SuperNovaAugmentedCircuit::new(         &pp.augmented_circuit_params_secondary,
//         None,
//         c_secondary,
...
where E1: CurveCycleEquipped {
  ...
  // This is mostly done with the existing Nova code. With additions of U_i[] and program_counter
  // checks in the augmented circuit.
...
fn test_supernova_pp_digest() {
  ...
  test_pp_digest_with::<PallasEngine, _>(&test_rom, &expect![
    "698b3592bf271c0cc53245aee71ec3f8e0d16486b3efc73be290a0af27605b01"
  ]);
  ...
  test_pp_digest_with::<Secp256k1Engine, _>(&test_rom_secp, &expect![
    "c94ee4e2870e34d6d057aa66157f8315879ecf2692ab9d1e2567c5830bed1103"
  ]);
}
...
impl<F: PrimeField> RootCheckingCircuit<F> {
  fn new(num_steps: usize) -> (Vec<F>, Vec<Self>) {
    ...
    for i in 0..num_steps + 1 {
      ...
      // Cube-root and fifth-root circuits alternate. We compute the hints backward, so the
      // calculations appear to be associated with the 'wrong' circuit. The final circuit is
      // discarded, and only the final seed is used (as z_0).
...
where E1: CurveCycleEquipped
{
  ...
  fn secondary_circuit(&self) -> Self::C2 { TrivialSecondaryCircuit::<E1::Base>::default() }
}

...
impl RustCryptoBackend13 {
    ...
    pub fn insert_record(&mut self, record_meta: RecordMeta) {
        self.record_map
            .insert(record_meta.nonce.clone(), record_meta);
    }
    ...
    pub fn derive_keys(&mut self) -> TlsKeys {
        ...
        let current = self
            .hkdf_provider
            .extract_from_zero_ikm(Some(okm_block.as_ref()));
            ...
            .extract_from_secret(Some(k0_salt.as_ref()), &self.pre_master_secret.unwrap());
        {
            let hs_tag = self
                .hkdf_provider
                .hmac_sign(&k0_salt, &self.pre_master_secret.unwrap().clone());
        ...
        let (master_secret, client_label, server_label) =
            if matches!(self.encrypt_mode, EncryptMode::Application) {
                ...
                let k1_secret = self
                    .hkdf_provider
                    .extract_from_secret(Some(k1_salt.as_ref()), &[0u8; 32]);
        ...
        debug!(
            "intermediate witness set during encrypt_mode: {:?}",
            self.encrypt_mode
        );
...
impl Backend for RustCryptoBackend13 {
    ...
    fn get_pre_master_secret(&mut self) -> Option<[u8; 32]> {
    ...
    // There are features in TLS 1.3 which this does not support (early data, resumption, ech, key update)
    ...
    async fn get_server_finished_vd(&mut self, hash: Vec<u8>) -> Result<Vec<u8>, BackendError> {
        let e = self
            .hkdf_provider
            .expander_for_okm(&self.server_hs_secret.clone().unwrap());
    ...
    async fn get_client_finished_vd(&mut self, hash: Vec<u8>) -> Result<Vec<u8>, BackendError> {
        let e = self
            .hkdf_provider
            .expander_for_okm(&self.client_hs_secret.clone().unwrap());
        let hmac_key = hkdf_expand_label_block(e.as_ref(), b"finished", &[]);
        let r = self.hkdf_provider.hmac_sign(&hmac_key, &hash);
    ...
    fn set_encrypt_decrypt(&mut self, mode: EncryptMode) -> Result<(), BackendError> {
        ...
        if matches!(
            self.encrypt_mode,
            EncryptMode::Application | EncryptMode::Handshake
        ) {
    ...
    fn set_hs_hash_client_key_exchange(&mut self, hash: Vec<u8>) -> Result<(), BackendError> {
        trace!(
            "Setting Client KX Hash: {:x?}",
            BASE64_STANDARD.encode(hash.clone())
        );
        ...
        let meta = match typ {
            ContentType::Handshake => {
                ...
                if plaintext[0] == 20u8 {
                    ...
                    let meta =
                        RecordMeta::new(&aad, "SF", &plaintext, &m.payload.0, &nonce.to_vec());

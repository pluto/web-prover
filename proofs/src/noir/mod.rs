...
pub struct NoirFCircuit {
  ...
  pub circuit:             Circuit<GenericFieldElement<bn256::Fr>>,
  pub state_len:           usize,
  ...
  fn new(params: Self::Params) -> Result<Self, Error> {
    ...
    let input_string = match source {
      PathOrBin::Path(path) => {
        ...
        std::fs::read(&file_path).map_err(|_| {
          Error::Other(format!(
            "{} is not a valid path\nRun either `nargo compile` to generate missing build \
             artifacts or `nargo prove` to construct a proof",
            file_path.display()
          ))
        })?
      },
    ...
    Ok(NoirFCircuit { circuit, state_len, external_inputs_len })
  }

  fn state_len(&self) -> usize { self.state_len }

  fn external_inputs_len(&self) -> usize { self.external_inputs_len }
  ...
  fn generate_step_constraints(
    ...
    self
      ...
      .map(|witness| {
        ...
        let value = if val == F::zero() { "0".to_string() } else { val.to_string() };

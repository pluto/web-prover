...
//
// The difference between these structures and the ACIR structure that the compiler uses is the
// following:
  ...
  fn from(
    ...
    let values: BTreeMap<Witness, _> = (0..num_variables)
      .map(|witness_index| {
        ...
        let value = witness_map.get(&witness).map_or(F::zero(), |field| field.into_repr());

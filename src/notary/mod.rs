...
use pki_types::{ServerName, CertificateDer};
use rustls::{ClientConfig, RootCertStore};
use serde::{Deserialize, Serialize};
// use tokio::net::TcpStream;
// use tokio_rustls::TlsConnector;
use tokio_util::bytes::Bytes; // which bytes
...
#[instrument(
  level = "trace",
  skip(host, port, max_sent_data, max_recv_data, notary_ca_cert_path, notary_ca_cert_server_name)
)]
pub async fn request_notarization(
  ...
  let (mut request_sender, connection) = {
    ...
    let _enter = span.enter();
    trace!("Connecting to notary server with client notary config: {:?}", client_notary_config);
  ...
  let notarization_response = {
    ...
    let _enter = span.enter();
    trace!(
      "Sending configuration for notarization session request with max_sent_data: {:?}, \
       max_recv_data: {:?}",
      max_sent_data,
      max_recv_data
    );
    ...
    trace!("Received configuration response: {:?}", configuration_response);
  ...
  Ok((notary_tls_socket.into_inner(), notarization_response.session_id))
}
